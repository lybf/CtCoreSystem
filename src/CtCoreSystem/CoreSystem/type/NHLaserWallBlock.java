...
//引用新视界代码
public class NHLaserWallBlock extends Block {
    ...
    public NHLaserWallBlock(String name) {
        super(name);
        config(Integer.class, (Cons2<NHLaserWallBuild, Integer>) Linkablec::linkPos);
        ...
        consumePowerCond(30f, NHLaserWallBuild::canActivate);
    }
    ...
    public void drawPlace(int x, int y, int rotation, boolean valid) {
        ...
        if (selected == null || !(selected.block instanceof NHLaserWallBlock) || !(selected.within(xB, yB, range)))
            return;
    ...
    public class NHLaserWallBuild extends Building implements Linkablec {
        protected transient NHLaserWallBuild target;
        ...
        public boolean linkValid(Building b) {
            return b instanceof NHLaserWallBuild && b.team == team && b.isValid() && ((NHLaserWallBuild) b).link() != this;
        }
        ...
        public void linkPos(int value) {
            ...
            if (linkValid(world.build(linkPos))) {
                target = (NHLaserWallBuild) world.build(linkPos);
            } else {
                target = null;
                linkPos = -1;
            }
    ...
    public static class Shooter extends BulletType {
        ...
        public void draw(Bullet b) {
            if (!(b.data instanceof NHLaserWallBuild)) return;
            NHLaserWallBuild build = (NHLaserWallBuild) b.data();
