...
//同个方块的数量限制
public class PowerTurretLimit {

    //普通炮塔
    public static class 炮塔数量限制 extends PowerTurret {
        //  数字代表允许建筑的数量，最多是数字的值+1
        public int 数量 = 1;

        public 炮塔数量限制(String name) {
            super(name);
            数量 = 1;
        }
    ...
    public static class 炮塔前置限制 extends PowerTurret {
        // 前置方块存在场上的数量，低于数字的值则不允许建造
        public int 数量 = 1;

        public 炮塔前置限制(String name) {
            super(name);
            前置方块 = Blocks.router;
            数量 = 1;
        }
