            ...
            if (Vars.state.isCampaign() && !enable) {
               ...
               // if (skip != null) {
                    skip.clicked(() -> {
                        ...
                        showTips(displayTime, Core.bundle.get("warning.Waves") + String.format("%.2f", multiple * 10) + "%");
                    });
                // }
/*                else{
    ...
    private void showTips(float sec, String msg) {
        sec *= 60.0F;
        ...
        tabl.collapser((top) -> {
            top.background(Styles.black6).add(msg).pad(8.0F).update((label) -> {
                label.color.set(Color.white).lerp(Color.red, Mathf.absin(Time.time, 4.0F, 2.0F));
            });
        }, true, () -> {
