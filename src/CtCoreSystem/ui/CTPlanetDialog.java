...
public class CTPlanetDialog extends BaseDialog implements PlanetInterfaceRenderer{
    ...
    public CTPlanetDialog(){
        ...
        addListener(new InputListener(){
            ...
            public boolean keyDown(InputEvent event, KeyCode key){
                if(event.targetActor == CTPlanetDialog.this && (key == KeyCode.escape || key == KeyCode.back || key == Binding.planetMap.value.key)){
    ...
    void displayItems(Table c, float scl, ObjectMap<Item, ExportStat> stats){
        displayItems(c, scl, stats, "产出", t -> {}, true);
    }
...
//什么玩意的颜色
    ...
    void playSelected(){
        ...
        for(Planet planet : content.planets()){
            if(!planet.allowWaveSimulation && !debugSelect && planet.allowWaveSimulation == sector.planet.allowWaveSimulation){
                ...
                if(attacked != null &&  planet.sectors.count(Sector::isAttacked) < 2 && !settings.getBool("forceIgnoreAttack") ){
    ...
    public Element getLastArcAnnouncement() {
        return lastArcAnnouncement;
    }
